...
public class WhereClauseTest {
    ...
    public void testEqual() {
        ...
        try {
            ...
            Assert.assertTrue(
                new WhereClause("STATE_ABBR in ('AK','AZ','CA','CO','HI','ID','MT','NM','NV','OR','UT','WA','WY')")
                    .accepts(getTestCase1()));
            Assert.assertTrue(new WhereClause("a=1 and (b=2 or c='foo')").accepts(getTestCase1()));
            Assert.assertTrue(new WhereClause("(a=1 and (b=2 or c='foo'))").accepts(getTestCase1()));
            Assert.assertTrue(new WhereClause("c='foo' AND ((a >= 0.5 AND a <= 1.5) OR (b > 1.5 AND b <= 2.5))")
                .accepts(getTestCase1()));
    ...
    public void testSubExpressions() {
        try {
            ...
            Assert.assertTrue(!new WhereClause("a=2 or (b<0 and c='foo')").accepts(getTestCase1()));
            Assert.assertTrue(
                new WhereClause("a=2 or (b=0 and c='foo') or STATE_NAME in(California)").accepts(getTestCase1()));
            Assert.assertTrue(
                new WhereClause("a=2 or (b=0 and STATE_NAME in('California')) or c='foo'").accepts(getTestCase1()));
            Assert.assertTrue(
                !new WhereClause("a=2 or (b=0 and STATE_NAME in(California)) and c='foo'").accepts(getTestCase1()));
            Assert.assertTrue(
                new WhereClause("a=2 or (b=0 and (STATE_NAME in(California) and STATE_ABBR='UT')) or c='foo'")
                    .accepts(getTestCase1()));
